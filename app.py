...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = X * 0 + np.random.normal(mu, np.sqrt(sigma2), N).reshape(-1, 1)  # Replace with code to generate random values for Y with specified mean and variance
    ...
    for _ in range(S):
        ...
        Y_sim = X_sim * 0 + np.random.normal(mu, np.sqrt(sigma2), N).reshape(-1, 1)
